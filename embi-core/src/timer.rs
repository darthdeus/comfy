...
// Most code taken from bevy's timer. It doesn't exactly math the latest API which switched from
// `repeating: bool` to enums.
...
impl Timer {
    ...
    pub fn tick(&mut self, delta: Duration) -> &Self {
        ...
        if self.finished() {
            if self.repeating() {
                self.times_finished = (self.elapsed().as_nanos() /
                    self.duration().as_nanos())
                    as u32;
                // Duration does not have a modulo
                self.set_elapsed(
                    self.elapsed() - self.duration() * self.times_finished,
                );
            } else {
